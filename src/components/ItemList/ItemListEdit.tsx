...
import { convertCategoryToNumber, disabledCategories, getFullItemList } from './getFullItemList';
...
const ItemListEdit: FC<ItemListEditProps> = ({ setItems, items }) => {
  ...
  return (
    <div className={styles.container}>
      {getFullItemList()
        .filter(({ name }) => !disabledCategories.includes(convertCategoryToNumber(name)))
        .map(({ name, items: itemsList }) => {
            ...
            <div key={`${name}-edit`}>
              ...
              <ul className={styles.list}>
                ...
                {itemsList.map((item) => {
                  ...
                  const checkValue =
                    categoryNumber in items && items[categoryNumber]?.includes(item);
                  return (
                    <li
                      key={`${name}-${item}-edit`}
